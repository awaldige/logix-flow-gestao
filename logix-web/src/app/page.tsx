      ...
      {isFormOpen && (
        <div className="fixed inset-0 bg-black p-10">
          <form onSubmit={handleSubmit} className="bg-zinc-900 p-6 space-y-4">
            ...
            <h2>{editingId ? '✏️ Editando' : '➕ Novo'}</h2>
            ...
            {tab === 'frota' && (
              <>
                ...
                <select value={formData.status || 'DISPONIVEL'} onChange={e => setFormData({...formData, status: e.target.value})}>
            ...
            {tab === 'motoristas' && (
              <>
                ...
                <select value={formData.status || 'ATIVO'} onChange={e => setFormData({...formData, status: e.target.value})}>
            ...
            {tab === 'viagens' && (
              <>
                ...
                <textarea placeholder="Observações" value={formData.obs || ''} onChange={e => setFormData({...formData, obs: e.target.value})}/>
              </>
            )}
